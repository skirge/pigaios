...
def guess_function_names(strings_list):
  ...
  for key in func_names:
    ...
    tmp = idc.demangle_name(func_name, INF_SHORT_DEMNAMES)
    if tmp is not None:
      func_name = tmp
...
class CBinaryToSourceExporter:
  ...
  def create_database(self, sqlite_db = None):
    if sqlite_db is None:
      sqlite_db = os.path.splitext(idc.get_idb_path())[0] + "-src.sqlite"
    ...
    if len(drefs) > 0:
      for dref in drefs:
        if get_func(dref) is None:
          ...
          str_constant = ida_bytes.get_strlit_contents(dref, -1, -1)
